...
const trendModalCloseBtn = document.getElementById('trendModalCloseBtn');
...
categories.forEach(cat => {
    ...
    trendCategorySelect.appendChild(option);
});
...
// Set default country
trendCountrySelect.value = 'us';


// --- Main Function to Fetch and Display Trends ---
async function fetchAndDisplayTrends() {
    const category = trendCategorySelect.value;
    const country = trendCountrySelect.value;

    trendList.innerHTML = '<p class="text-gray-400">Fetching trends...</p>';

    try {
        // Call our new serverless function
        const response = await fetch(`/api/trends?category=${category}&country=${country}`);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();

        trendList.innerHTML = ''; // Clear the list

        if (data.headlines && data.headlines.length > 0) {
            data.headlines.forEach(headline => {
                const trendItem = document.createElement('div');
                trendItem.className = 'p-3 hover:bg-gray-700 rounded-lg cursor-pointer';
                trendItem.textContent = headline;
                trendList.appendChild(trendItem);
            });
        } else {
            trendList.innerHTML = '<p class="text-gray-400">No trends found for this selection.</p>';
        }
    } catch (error) {
        console.error("Failed to fetch trends:", error);
        trendList.innerHTML = `<p class="text-red-400">Failed to load trends. Check the console for details.</p>`;
    }
}

// --- Event Listeners ---
findTrendBtn.addEventListener('click', () => {
    trendModal.classList.remove('hidden');
    fetchAndDisplayTrends(); // Fetch trends when modal opens
});

trendModalCloseBtn.addEventListener('click', () => {
    trendModal.classList.add('hidden');
});

// Refetch trends when a filter is changed
trendCategorySelect.addEventListener('change', fetchAndDisplayTrends);
trendCountrySelect.addEventListener('change', fetchAndDisplayTrends);

// Handle clicking on a trend in the list
trendList.addEventListener('click', (e) => {
    if (e.target && e.target.classList.contains('p-3')) {
        const selectedTrend = e.target.textContent;

        // Ensure the custom input field is visible
        customSubjectInput.classList.remove('hidden');
        manualSubjectSelect.classList.add('hidden');

        // As planned, paste the trend into the custom subject field
        customSubjectInput.value = selectedTrend;
